...
const quotaToBeUpdated = ref<number>(props.user.flightsQuota)
const selectedReasonId = ref<number>(0)
const flightsQuotaUpdateReasons = ref<{ add: { id: number, message: string }[], remove: { id: number, message: string }[] }>({ add: [], remove: [] })

const flightQuotaSelectOptions = computed(() => {
  if(quotaToBeUpdated.value === props.user.flightsQuota) return []
  if(quotaToBeUpdated.value > props.user.flightsQuota) return flightsQuotaUpdateReasons.value.add
  return flightsQuotaUpdateReasons.value.remove
})

const isSaveButtonDisabled = computed(() => quotaToBeUpdated.value === props.user.flightsQuota || !selectedReasonId.value || loading.value)
...
const saveUser = async () => {
  await updateUserFlightQuota(
    { ...props.user, flightsQuota: quotaToBeUpdated.value },
...
onMounted(async () => {
  ...
  retrieveQuotaChanges(props.user.id)
})
</script>
